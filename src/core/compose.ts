...
export const DEFAULT_IMAGE = 'xrpllabsofficial/xrpld:3.2.0';
...
function generateStandaloneYaml(
  ...
  // xrpllabsofficial/xrpld 3.2.0+ moved the binary to /usr/bin/xrpld;
  // rippleci/rippled still uses the legacy /opt/ripple/bin/rippled path.
  const isOfficialImage = image.startsWith('xrpllabsofficial/xrpld');
  const RIPPLED_BIN = isOfficialImage ? '/usr/bin/xrpld' : '/opt/ripple/bin/rippled';
  ...
  const needsConfFlag = !isOfficialImage;
...
function generateConsensusYaml(
  ...
  const entrypointPrimary =
    `["/bin/bash", "-c", "if [ -f /var/lib/xrpld/db/ledger.db ]; then exec /entrypoint.sh --load; else exec /entrypoint.sh --start; fi"]`;
  const entrypointPeer =
    `["/bin/bash", "-c", "if [ -f /var/lib/xrpld/db/ledger.db ]; then exec /entrypoint.sh --load; else exec /entrypoint.sh; fi"]`;
