...
42EEA5E28A97824821D4EF97081FE36A54E9593C6E4F20CBAE098C69D2E072DC FeeEscalation
...
B4D44CC3111ADD964E846FC57760C8B50FFCD5A82C86A72756F6B058DDDF96AD NegativeUNL
...
export function writeComposeFile(image = DEFAULT_IMAGE, persist = false, debug = false, ledgerIntervalMs = 0, configPath?: string, noRestart = false): string {
  ...
  const entrypointLine = noRestart
    ? '\n    entrypoint: ["/bin/sh", "-c", "/opt/ripple/bin/rippled --conf /config/rippled.cfg -a --start 2>/tmp/rip.err & RPID=$! ; wait $RPID ; EC=$? ; cat /tmp/rip.err >&2 ; grep -qF Logic\\ error: /tmp/rip.err 2>/dev/null && exit 134 ; exit $EC"]'
    : '';
  const commandLine = noRestart
    ? ''  // the entrypoint already contains the full rippled invocation
    : '\n    command: ["-a", "--start"]';
