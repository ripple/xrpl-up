...
export function writeComposeFile(image = DEFAULT_IMAGE, persist = false, debug = false, ledgerIntervalMs = 0, configPath?: string, noRestart = false): string {
  ...
  // creates a fresh genesis ledger even when an existing NuDB database is
  // present, wiping all persisted state.  Use a shell entrypoint that passes
  // --start only when the NuDB directory is empty (first boot) and omits it
  // on subsequent boots so rippled continues from the existing ledger.
