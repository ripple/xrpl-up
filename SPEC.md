...
Known gaps, not covered — all three connect via their own direct `new Client(...)` rather than `withClient`, so none get the `networkID`-based block above (accepted as-is, not planned to be fixed):
...
- `status` and `accounts` — both connect via `NetworkManager` (`src/core/network.ts`), a separate connection path from `withClient`. `faucet` and `start` also use `NetworkManager` but already have their own independent `isMainnet()` hostname gate, so those two are unaffected.
