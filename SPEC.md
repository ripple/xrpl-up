...
Known gap, not yet covered: the scaffolded example scripts generated by `xrpl-up init` (`src/commands/init.ts`) construct `new Client(process.env.XRPL_NETWORK_URL ?? ...)` directly rather than importing `withClient` from the `xrpl-up` package, so they bypass this check if run standalone (not via `xrpl-up run`) with `XRPL_NETWORK_URL` pointed at mainnet. Fixing this means either rewriting the templates to use `withClient`, or adding the same check inline — not done as part of this change.
